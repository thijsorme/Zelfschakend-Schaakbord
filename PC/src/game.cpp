...
void Game::handleEvents() {
    ...
    switch (evnt.type) {
        ...
        case SDL_MOUSEBUTTONDOWN:
            ...
            break;
        case SDL_KEYDOWN:
            for (auto const& [key, val] : input.keyPressed)
            {
                if(evnt.key.keysym.sym == val.SDLKey){
                    std::cout << key << std::endl;
                    input.keyPressed[key].value = true;
                    input.saveMousePos();
                    break;
                }
            } 
            for (auto const& [key, val] : input.modKeyPressed)
            {
                if(evnt.key.keysym.sym == val.SDLKey){
                    input.modKeyPressed[key].value = true;
                    input.saveMousePos();
                    break;
                }
            }
            ...
            break;
            //     if(evnt.key.keysym.sym == input.buttonsPressed){
        ...
        case SDL_KEYUP:
            ...
            {
                if(evnt.key.keysym.sym == val.SDLKey){
                    input.modKeyPressed[key].value = false;
                    input.saveMousePos();
                    break;
                }
            }
